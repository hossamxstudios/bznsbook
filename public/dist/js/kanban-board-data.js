...
const pipelineId = url2.split("/")[5];
const url        = `/apiss/deals/${pipelineId}/stages`;
function updateCounter(stageId,source){
    var old_stage_id = (source.id).substring(1);
    var new_element = document.getElementById("i"+stageId);
    var old_element = document.getElementById("i"+old_stage_id);
    document.getElementById("count"+stageId).innerHTML = (new_element.childElementCount - 1 );
    document.getElementById("count"+old_stage_id).innerHTML = (old_element.childElementCount - 1 );
}
$.ajax({
    url: url,
    type: "GET",
    success: function (data) {
        ...
        drake.on('drop', function (el, target, source, sibling) {
	        var dealId = el.id;
	        var stagelId =( target.id).substring(1);
            var updateUrl = `/deals/update-stage`;
            var data = {
                deal_id: dealId,
                stage_id: stagelId // Ensure CSRF token is available
            };
            $.ajax({
                url:updateUrl , // Replace with your route name or URL
                type: 'GET',
                data: data,
                success: function(response) {
                    console.log(source.id);
                    updateCounter(stagelId, source);
                },
                error: function(xhr, status, error) {
                    // Handle error
                    console.error('Failed to update deal stage:', error);
                    // Optionally, move the card back to its original container if update fails
