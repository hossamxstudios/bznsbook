...
$(function() {
    ...
    $('.gt-todo-table tbody tr').each(function() {
        $('<span class=""><input type="checkbox" class="form-check-input check-select" id="chk_sel_' + tdCnt + '"></span>').insertBefore($(this).find("td > .gt-single-task >div").eq(0));
        tdCnt++;
    });
	...
	$(document).on("shown.bs.tab",".todoapp-wrap .hk-pg-header a[data-bs-toggle='tab']",function (e) {
		var getOffset = $( ".bar" ).first();
        var offset = getOffset.offset().left;
        offset = offset/4;
        setTimeout(function() {
			s.change_view_mode("Week");
