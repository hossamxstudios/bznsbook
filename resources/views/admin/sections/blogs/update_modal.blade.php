...
<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasUpdate{{ $blog->id }}" aria-labelledby="offcanvasUpdateLabel" style="width:620px;">
    <div class="offcanvas-header" style="background: #474e5d;">
        <h5 id="offcanvasUpdateLabel" style="color:aliceblue">{{ x_('Update Blog', 'blogs') }}</h5>
    ...
    <div class="offcanvas-body">
        <form action="{{ route('blogs.update', ['id' => $blog->id]) }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
            ...
            <div class="mb-3 position-relative">
                <label for="topic_id_{{ $blog->id }}" class="form-label">{{ x_('Topic', 'blogs') }} <span class="text-danger">*</span></label>
                <select class="form-select @error('topic_id') is-invalid @enderror" id="topic_id_{{ $blog->id }}" name="topic_id" required>
                    <option value="" disabled>{{ x_('Select Topic', 'blogs') }}</option>
            ...
            <div class="mb-3 position-relative">
                <label for="title_{{ $blog->id }}" class="form-label">{{ x_('Blog Title', 'blogs') }} <span class="text-danger">*</span></label>
            ...
            <div class="mb-3 position-relative">
                <label for="sub_title_{{ $blog->id }}" class="form-label">{{ x_('Blog Sub Title', 'blogs') }} <span class="text-danger">*</span></label>
            ...
            <div class="mb-3 position-relative">
                <label for="image_{{ $blog->id }}" class="form-label">{{ x_('Blog Image', 'blogs') }}</label>
                @if($blog->getFirstMedia('images'))
                    <div class="mb-2">
                        ...
                        <p class="small">{{ x_('Current image', 'blogs') }}</p>
                    </div>
                @endif
                ...
                <small class="form-text text-muted">{{ x_('Upload a new blog image (max 2MB) or leave empty to keep the current one. Recommended size: 1200x630px.', 'blogs') }}</small>
                @error('image')
                    <div class="invalid-feedback">{{ $message }}</div>
                @else
            ...
            <div class="mb-4 position-relative">
                <label for="details_{{ $blog->id }}" class="form-label">{{ x_('Blog Content', 'blogs') }} <span class="text-danger">*</span></label>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary">
                    ...
                    <span>{{ x_('Update Blog', 'blogs') }}</span>
                </button>
            </div>
        </form>
        <!-- End Form -->
