                ...
                <div class="col-md-12 mb-md-4 mb-3">
                    <div class="card rounded-8 mb-0">
                        <div class="card-header card-header-action">
                            <h6>{{ x_('Users And Roles', 'admin') }} <span class="badge badge-sm badge-light ms-1">{{$users->count()}}</span></h6>
                        </div>
                        <div class="card-body">
                                        ...
                                        @foreach ($users as $user)
                                            <tr class="hover-row">
                                                <td style="background: white">
                                                    <button type="button" class="btn btn-link p-0 view-details-btn" data-bs-toggle="offcanvas" data-bs-target="#offcanvasShow{{$user->id}}" title="{{ x_('View Details', 'admin') }}">
                                                            {{ $user->name }}
                                                    </button>
                                                    ...
                                                    <div role="menu" class="dropdown-menu dropdown-menu-end">
                                                        <a class="dropdown-item delete-tasklist" href="#" data-bs-toggle="modal" data-bs-target="#roleModal{{$user->id}}" >{{ x_('Edit User & Role', 'admin') }}</a>
                                                        <a class="dropdown-item delete-tasklist" href="#" data-bs-toggle="modal" data-bs-target="#deleteModal{{$user->id}}" >{{ x_('Delete User', 'admin') }}</a>
                                                    </div>
                                                </td>
                                                <td style="background: white">
                                                    {{ $user->email }}
