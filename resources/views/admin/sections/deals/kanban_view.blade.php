            ...
            <div class="card card-flush tasklist">
                <div class=" card-header card-header-action h-100 m-0 py-1" style="border-right:0.4px solid gray;">
                    <div class="tasklist-handle m-0 p-0 h-100"              style="flex-direction: column; height:80px!important;justify-content: flex-start;align-content: flex-start;align-items: flex-start;">
                        ...
                        <p class="align-items-center mb-0 py-1"             style="font-size: 0.78rem;"><span class="tasklist-name">{{ x_('Weighted:', 'deals') }}</span><span class="badge badge-sm badge-pill badge-light ms-2">${{$stage->deals?->sum('amount')*($stage->percentage/100)}}</span></p>
                        <p class="align-items-center mb-0"                  style="font-size: 0.9rem;"><span class="tasklist-name">{{ x_('Total:', 'deals') }}</span><span class="badge badge-sm badge-pill badge-light ms-2">${{$stage->deals?->sum('amount')}}</span></p>
                    </div>
                </div>
                    ...
                    <div id="i{{ $stage->id }}" class="tasklist-cards-wrap">
                                ...
                                <div class=" card-header card-header-action rounded-8" style="display: flex;justify-content: space-between; align-items: center;">
                                    ...
                                    <div class="">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <a class="dropdown-item " href={{ route('deals.markaspaid', $deal->id) }} title="{{ x_('Mark as Paid', 'deals') }}"><span class="feather-icon dropdown-icon"><i data-feather="dollar-sign"></i></span><span>{{ x_('Mark as Paid', 'deals') }}</span></a>
                                            <a class="dropdown-item " href="#" data-bs-toggle="offcanvas" data-bs-target="#offcanvasShow{{$deal->id}}" title="{{ x_('View Details', 'deals') }}"><span class="feather-icon dropdown-icon"><i data-feather="eye"></i></span><span>{{ x_('Show', 'deals') }}</span></a>
                                            <a class="dropdown-item" href="#" data-bs-toggle="offcanvas" data-bs-target="#offcanvasUpdate{{$deal->id}}"><span class="feather-icon dropdown-icon"><i data-feather="edit-2"></i></span><span>{{ x_('Edit', 'deals') }}</span></a>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-body pb-0">
                                    <p style="font-size: 1.1rem;"> <span style="color:black">{{ x_('Amount:', 'deals') }}</span> ${{$deal->amount}} </p>
                                </div>
                                <div class="card-footer text-muted justify-content-between">
                                    <div>
                                        <span class="task-deadline" style="font-size: 1.1rem;">
                                            <span style="color:black">{{ x_('Close Date:', 'deals') }}</span>
                                            {{$deal->closed_at ? date('d-M-Y ',strtotime($deal->closed_at)) : x_('N/A', 'admin')}}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card card-simple card-flush rounded-8 tasklist-card" style="opacity: 0" >
                            <div class=" card-header card-header-action rounded-8" style="opacity: 0">
                                <h6 class="fw-semibold">{{ x_('No Deals', 'deals') }}</h6>
                            </div>
                        </div>
                    </div>
                </div>
