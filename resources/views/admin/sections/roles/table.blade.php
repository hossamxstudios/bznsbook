                ...
                <div class="col-md-12 mb-md-4 mb-3">
                    <div class="card rounded-8 mb-0">
                        <div class="card-header card-header-action">
                            <h6>{{ x_('Roles And Permissions', 'roles') }} <span class="badge badge-sm badge-light ms-1">{{$roles->count()}}</span></h6>
                        </div>
                        <div class="card-body">
                                        ...
                                        @foreach ($roles as $role)
                                            <tr class="hover-row">
                                                <td style="background: white">
                                                    <button type="button" class="btn btn-link p-0 view-details-btn" data-bs-toggle="offcanvas" data-bs-target="#offcanvasShow{{$role->id}}" title="{{ x_('View Details', 'roles') }}">
                                                            {{ $role->name }}
                                                    </button>
                                                    ...
                                                    <div role="menu" class="dropdown-menu dropdown-menu-end">
                                                        <a href="{{ route('roles.update_view', ['id' => $role->id]) }}" class="dropdown-item">{{ x_('Edit', 'roles') }}</a>
                                                        <a class="dropdown-item delete-tasklist" href="#" data-bs-toggle="modal" data-bs-target="#deleteModalgrid{{$role->id}}" >{{ x_('Delete', 'roles') }}</a>
                                                    </div>
                                                </td>
                                                @php
                                                    $count = 0;
