@foreach($companies as $company)
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="servicesModal{{ $company->id }}" aria-labelledby="servicesOffcanvasLabel{{ $company->id }}" style="width:870px;">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="servicesOffcanvasLabel{{ $company->id }}">{{ x_('Manage Sections for', 'companies') }} {{ $company->name }}</h5>
        ...
        <div class="offcanvas-body">
            <form action="{{ route('company.services.update', $company->id) }}" method="POST">
                ...
                <div class="mb-3 form-group">
                    <label class="form-label">{{ x_('Select Services', 'companies') }}</label>
                ...
                <div class="d-flex justify-content-end">
                    <button type="button" class="btn btn-secondary me-2" data-bs-dismiss="offcanvas">{{ x_('Close', 'companies') }}</button>
                    <button type="submit" class="btn btn-primary">{{ x_('Save changes', 'companies') }}</button>
                </div>
            </form>
        </div>
    </div>
