<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasUpdate{{$company->id}}" aria-labelledby="offcanvasUpdateLabel" style="width:570px;">
    <div class="offcanvas-header" style="background: #474e5d;">
        <h5 id="offcanvasUpdateLabel" style="color:aliceblue">{{ x_('Update Company', 'general') }}</h5>
    ...
    <div class="offcanvas-body">
        <form action="{{ route('companies.update', ['id' => $company->id]) }}" method="POST">
            @csrf
            <div class="mb-3">
                <label for="industry{{ $company->id }}" class="form-label">{{ x_('Industry*', 'general') }}</label>
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="capacity" name="capacity">
                    ...
                    <option value=""  >{{ x_('Select Company Headcount', 'general') }}</option>
            ...
            <button type="submit" class="btn btn-primary">{{ x_('Update Company', 'general') }}</button>
        </form>
    </div>
</div>
