                ...
                <div class="taskboardapp-wrap">
                    <div class="taskboardapp-content">
                        <div class="taskboardapp-detail-wrap">
                            @include('company.sections.interviews.topbar')
                            <div class="taskboard-body tab-content ">
                                @include('company.sections.interviews.table')
                            </div>
                        </div>
                        @include('company.sections.interviews.add_modal')
                        @foreach ($interviews as $interview)
                            @include('company.sections.interviews.show_modal')
                            {{-- @include('company.sections.interviews.add_tag') --}}
                            @include('company.sections.interviews.add_note')
                            @include('company.sections.interviews.add_attachment')
                            {{-- @include('company.sections.interviews.add_batch')  --}}
                            {{-- @include('company.sections.interviews.update_modal') --}}
                            @include('company.sections.interviews.delete_modal')
                        @endforeach
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('select[name="company_id"]').on('change', function () {
                company_id1 = $('select[name="company_id"]').val();
                if (company_id1) {
                    $.ajax({
                        url: "{{ URL::to('/api/company/careers/') }}/"+company_id1,
                        type: "GET",
                        dataType: "json",
